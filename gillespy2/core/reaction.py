...
class Reaction(SortableObject, Jsonify):
    ...
    def __init__(
        ...
        if self.propensity_function is not None and self.massaction:
            errmsg = (
                "Reaction {} You cannot set the propensity type to mass-action and simultaneously set a "
                "propensity function."
            ).format(self.name)
            ...
            def __customPropParser():
                ...
                class ExpressionParser(ast.NodeTransformer):
                    def visit_BinOp(self, node):
                        ...
                        if isinstance(node.op, (ast.BitXor, ast.Pow)):
                            ...
                            call = ast.Call(func=pow_func, args=[node.left, node.right], keywords=[])
                ...
                class ToString(ast.NodeVisitor):
                    ...
                    def visit_Call(self, node):
                        func_name = (
                            ToString.substitutions.get(node.func.id)
    ...
    def add_reactant(self, species, stoichiometry):
        ...
        :param species: Reactant to add to this reaction.
        ...
        if stoichiometry <= 0:
            raise ReactionError(
                "Reaction Stoichiometry must be a \
                                    positive integer."
            )
        self.reactants[species.name] = stoichiometry

    def add_product(self, species, stoichiometry):
        """
        Adds a product to the reaction (species that is created)

        :param species: Product to add to this reaction.
        :type species: gillespy2.Species
        :param stoichiometry: The stoichiometry of the given product.
        :type stoichiometry: int
        """
        self.products[species.name] = stoichiometry
    ...
    def sanitized_propensity_function(self, species_mappings, parameter_mappings, ode=False):
        ...
        for _id, name in enumerate(names):
            sanitized_propensity = sanitized_propensity.replace(name, "{" + str(_id) + "}")
