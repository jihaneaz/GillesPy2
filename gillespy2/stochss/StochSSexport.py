...
def __add_event_assignments(model, event, assignments):
    for assignment in assignments:
        ...
        s_assignment = {"variable": variable, "expression": assignment.expression}
...
def __add_parameters(model, parameters):
    for name, parameter in parameters.items():
        ...
        s_parameter = {"compID": model["defaultID"], "name": name, "expression": str(expression), "annotation": ""}
...
def __add_reactions(model, reactions):
    for name, reaction in reactions.items():
        ...
        for key in ["reactants", "products"]:
            __add_stoich_species(s_reaction=s_reaction, reaction=reaction, key=key, species=model["species"])
...
def __add_stoich_species(s_reaction, reaction, key, species):
    ...
    for specie, ratio in source.items():
        stoich_species = {"ratio": ratio, "specie": __get_species(species=species, name=specie.name)}
...
def export(model, path=None, return_stochss_model=False):
    ...
    s_model = {
        ...
        "annotation": "",
        "volume": model.volume,
        "modelSettings": {"endSim": model.tspan[-1], "timeStep": model.tspan[-1] / len(model.tspan)},
        "species": [],
        "initialConditions": [],
        "parameters": [],
        "reactions": [],
    ...
    __add_function_definitions(model=s_model, function_definitions=model.get_all_function_definitions())
