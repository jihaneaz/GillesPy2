...
def __read_sbml_model(filename):
    ...
    if error_count > 0:
        for i in range(error_count):
            ...
            errors.append(
                [
                    "SBML {0}, code {1}, line {2}: {3}".format(
                        error.getSeverityAsString(), error.getErrorId(), error.getLine(), error.getMessage()
                    ),
                    converter_code,
                ]
            )
...
def __get_math(math):
    ...
    replacements = {r"\bln\b": "log", r"\^": "**", r"\&\&": "and", r"\|\|": "or"}
...
def __get_reactions(sbml_model, gillespy_model, errors):
    ...
    for i in range(sbml_model.getNumReactions()):
        ...
        gillespy_reaction = gillespy2.Reaction(
            name=name, reactants=reactants, products=products, propensity_function=propensity
        )
    ...
    for i in range(sbml_model.getNumRules()):
        ...
        if rule_variable in gillespy_model.listOfParameters:
            ...
            species = gillespy2.Species(
                name=rule_variable, initial_value=value, allow_negative_populations=True, mode="continuous"
            )
        ...
        if rule.isAssignment():
            ...
            gillespy_rule = gillespy2.AssignmentRule(name=rule_name, variable=rule_variable, formula=rule_string)
        ...
        if rule.isRate():
            gillespy_rule = gillespy2.RateRule(name=rule_name, variable=rule_variable, formula=rule_string)
        ...
        if rule.isAlgebraic():
            ...
            errors.append(
                [
                    "{0} '{1}' found on line '{2}' with equation '{3}'. gillespy does not support SBML Algebraic Rules".format(
                        msg, rule.getId(), rule.getLine(), libsbml.formulaToString(rule.getMath())
                    ),
                    -5,
                ]
            )
...
def __get_events(sbml_model, gillespy_model):
    for i in range(sbml_model.getNumEvents()):
        ...
        for a in assignments:
            ...
            gillespy_assignment = gillespy2.EventAssignment(a.getVariable(), __get_math(a.getMath()))
            gillespy_assignments.append(gillespy_assignment)
        gillespy_event = gillespy2.Event(
            name=event.getId(),
            trigger=gillespy_trigger,
            assignments=gillespy_assignments,
            delay=delay,
