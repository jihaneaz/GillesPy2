...
help = """This script is an application wrapper for interfacing GillesPy2's ODE solver
...
""".format(
    gillespy2.__version__, gillespy2.__url__
)
...
with open(EXPECTED_RESULTS_FILE, "r") as expected_file:
    ...
    for spec in expected_species:
        if spec in model.listOfParameters:
            p_to_s = gillespy2.Species(
                name=spec,
                initial_value=model.listOfParameters[spec].value,
                mode="continuous",
...
if not len(model.listOfSpecies):
    ...
    for name, param in model.listOfParameters.items():
        species_to_add.append(
            gillespy2.Species(
                name=name, initial_value=param.expression, mode="continuous", allow_negative_populations=True
            )
        )
...
results = model.run(
    ...
    show_labels=True,
    integrator="Radau",
    integrator_options={"max_step": 0.25, "atol": atol, "rtol": rtol},
)
