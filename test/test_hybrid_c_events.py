...
class EventFeatures(unittest.TestCase):
    class BaseEventModel(gillespy2.Model):
        def __init__(self, s1, s2, rate):
            ...
            self.add_reaction(
                [gillespy2.Reaction(name="r1", rate=self.rate, reactants={self.s1: 1}, products={self.s2: 1})]
            )
    ...
    def test_delay_trigger_persistent(self):
        ...
        event1 = gillespy2.Event(
            ...
            trigger=gillespy2.EventTrigger(expression="S1<60 and S2<S1", persistent=False),
            delay="t+1.0",
        )
        event2 = gillespy2.Event(
            ...
            trigger=gillespy2.EventTrigger(expression="S2>90 and t<3.5", persistent=True),
            delay="1.0",
        )
