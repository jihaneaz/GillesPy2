...
class TestPropensityFunctions(unittest.TestCase):
    def test_propensity_functions(self):
        self.assertEqual(
            r1.propensity_function, "(((5*pow(x,2))+(2.718281828459045*b))+6)", msg="Has incorrect expression"
        )
        self.assertEqual(
            r2.propensity_function, "(((5*pow(x,2))+(2.718281828459045*b))+6)", msg="Has incorrect expression"
        )
        ...
        self.assertEqual(
            r7.propensity_function, "(((alpha/beta)+pow(delta,gamma))/(atlas-zeta))", msg="Has incorrect expression"
        )
        self.assertEqual(
            r8.propensity_function, "(((alpha/beta)+pow(delta,gamma))/(atlas-zeta))", msg="Has incorrect expression"
        )
    ...
    def test_dependency_graphing(self):
        ...
        correct_graph = {
            "cu": {"dependencies": ["cv", "dv"]},
            "cv": {"dependencies": ["cu", "du"]},
            "du": {"dependencies": ["cu"]},
            "dv": {"dependencies": ["cv"]},
        }
        ...
        dependencies = dependency_grapher(model, list(model.listOfReactions.keys()))
        correct_graph = {
            "r1": {"dependencies": ["r2", "r3"]},
            "r2": {"dependencies": ["r1", "r3"]},
