    ...
    def test_add_param_event(self):
        class EventTestModel(gillespy2.Model):
            def __init__(self):
                ...
                self.add_parameter(gillespy2.Parameter(name="event_tracker", expression=99))
                self.add_parameter(gillespy2.Parameter(name="event_tracker2", expression=0))
                self.add_reaction(
                    gillespy2.Reaction(name="r1", products={"S": 1}, rate=self.listOfParameters["event_tracker2"])
                )
                eventTrig1 = gillespy2.EventTrigger(expression="t>=2")
                event1 = gillespy2.Event(name="event1", trigger=eventTrig1)
                event1.add_assignment(gillespy2.EventAssignment(variable="event_tracker", expression="t"))
                eventTrig2 = gillespy2.EventTrigger(expression="t >= event_tracker + 2")
                event2 = gillespy2.Event(name="event2", trigger=eventTrig2)
                event2.add_assignment(gillespy2.EventAssignment(variable="event_tracker2", expression="t"))
    ...
    def test_continuous_state_values(self):
        ...
        for solver in self.solvers:
            with self.subTest(solver=solver.name):
                ...
                self.assertGreater(
                    result["S1"][-1], 0.0, "Reaction never fired; indicates that continuous species is being truncated"
                )
