    ...
    def test_run_output(self):
        ...
        for solver, trajectory_counts in self.solvers:
            for number_of_trajectories in trajectory_counts:
                with self.subTest(
                    "Processing simulation output for each solver with different trajectory sizes",
                    ...
                    for result in results:
                        ...
                        self.assertTrue(
                            np.allclose(tspan, expected_tspan),
                            msg="C++ Simulation output contains unexpected timeline values"
                            f"\n  Received timeline: {tspan}\n  Expected timeline: {expected_tspan}",
                        )
                        self.assertEqual(
                            first_value,
                            expected_init,
                            msg=f"C++ Simulation output begins with unexpected value: {first_value}",
                        )
                        self.assertAlmostEqual(
                            last_value,
                            0,
                            places=3,
                            msg=f"C++ Simulation output converges on an unexpectedly large value: {last_value}",
                        )
    ...
    def test_c_decoder(self):
        ...
        for decoder in TestCDecode.decoders:
            ...
            with self.subTest("Processing mock data output with C decoders", decoder=decoder):
                ...
                self.assertEqual(
                    time_stopped, 3, f"Stop time returned by decoder is incorrect (expected {3}, got {time_stopped})"
                )
