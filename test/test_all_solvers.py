...
class TestAllSolvers(unittest.TestCase):
    ...
    sbml_features = {
        "AssignmentRule": lambda model, variable: model.add_assignment_rule(
            gillespy2.AssignmentRule(variable=variable, formula="1/(t+1)")
        ),
        "RateRule": lambda model, variable: model.add_rate_rule(gillespy2.RateRule(variable=variable, formula="2*t")),
        "Event": lambda model, variable: model.add_event(
            gillespy2.Event(
                trigger=gillespy2.EventTrigger(expression="t>1"),
                assignments=[gillespy2.EventAssignment(variable=variable, expression="100")],
            )
        ),
        "FunctionDefinition": lambda model, variable: model.add_function_definition(
    ...
    def test_random_seed(self):
        for solver in self.solvers:
            with self.subTest(solver=solver.name):
                ...
                if solver.name in ["ODESolver", "ODECSolver"]:
                    continue
